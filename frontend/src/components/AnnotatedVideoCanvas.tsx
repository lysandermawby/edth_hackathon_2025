...
interface AnnotatedVideoCanvasProps {
  videoSrc: string
...
const findFrameForTime = (
  ...
  while (index < frames.length - 1 && frames[index + 1].timestamp <= time + tolerance) {
    index += 1
  }
...
const drawDetections = (ctx: CanvasRenderingContext2D, objects: TrackingObject[]): void => {
  objects.forEach(obj => {
    if (!obj.bbox) return
...
const AnnotatedVideoCanvas: React.FC<AnnotatedVideoCanvasProps> = ({ videoSrc, trackingData }) => {
  ...
  const preparedData = useMemo(() => prepareTrackingData(trackingData), [trackingData])
  ...
  useEffect(() => {
    ...
    const renderFrame = (): void => {
      if (video.readyState >= 2) {
        ...
        const frame = findFrameForTime(video.currentTime, preparedData.frames, preparedData.fps, lastFrameIndexRef)
...
export default AnnotatedVideoCanvas
