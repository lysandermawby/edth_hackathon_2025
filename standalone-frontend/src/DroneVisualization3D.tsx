...
const PropellerBlade: React.FC<{
  position: [number, number, number];
  spinning?: boolean;
}> = ({ position, spinning = true }) => {
...
const CameraGimbal: React.FC<{
  gimbalElevation?: number;
  gimbalAzimuth?: number;
}> = ({ gimbalElevation = 0, gimbalAzimuth = 0 }) => {
    ...
    <group ref={droneRef}>
      ...
      {armConfigs.map((config, index) => (
        <group key={index}>
          ...
          />
          <PropellerBlade
            position={config.propPos as [number, number, number]}
          />
        </group>
      ))}
      ...
      {/* Camera gimbal */}
      <CameraGimbal
        gimbalElevation={gimbal_elevation}
        gimbalAzimuth={gimbal_azimuth}
      />
    </group>
  );
};
...
const AxisHelper: React.FC = () => {
  return (
    <group position={[0, -1, 0]}>
      ...
      <Text position={[2.2, 0, 0]} fontSize={0.2} color="#ff4444">
        X
      </Text>
      ...
      <Text position={[0, 2.2, 0]} fontSize={0.2} color="#44ff44">
        Y
      </Text>
      ...
      <Text position={[0, 0, 2.2]} fontSize={0.2} color="#4444ff">
        Z
      </Text>
    </group>
  );
...
const TelemetryHUD: React.FC<{ metadata: DroneMetadata }> = ({ metadata }) => {
  ...
  return (
    <group
      ref={hudRef}
      position={[-viewport.width * 0.4, viewport.height * 0.35, 0]}
    >
      <Text
        fontSize={0.15}
        color="#00ffff"
...
const Scene: React.FC<{ metadata?: DroneMetadata }> = ({ metadata }) => {
  ...
  return (
    <>
      ...
      {/* Grid floor */}
      <primitive
        object={new THREE.GridHelper(10, 20, "#003333", "#001111")}
        position={[0, -1, 0]}
      />
...
const DroneVisualization3D: React.FC<DroneVisualization3DProps> = ({
  ...
  return (
    <div
      className={`${className} bg-cyber-black border border-cyber-border relative overflow-hidden`}
    >
      <Canvas
