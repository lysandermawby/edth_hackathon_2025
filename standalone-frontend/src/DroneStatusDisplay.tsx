...
const DroneStatusDisplay: React.FC<DroneStatusDisplayProps> = ({
  ...
  const GimbalIndicator: React.FC<{ elevation?: number; azimuth?: number }> = ({
    elevation = 0,
    azimuth = 0,
  }) => {
    return (
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="space-y-4 flex-1">
          ...
          >
              ...
              <div className="mt-2 space-y-1 text-xs font-mono">
                <div className="flex justify-between">
                  ...
                  <span className="text-neon-pink">
                    {formatAngle(metadata.gimbal_elevation)}°
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cyber-muted">AZIM:</span>
                  <span className="text-neon-pink">
                    {formatAngle(metadata.gimbal_azimuth)}°
                  </span>
                </div>
              </div>
            </div>
            ...
            {hasEnhancedTelemetry && session?.enhanced_telemetry && (
              <div>
                <h4 className="font-medium text-neon-green flex items-center gap-2 font-mono text-sm mb-2">
                  <HiWifi className="text-neon-green w-3 h-3" />
                  ENHANCED
                </h4>
                <div className="text-center">
                  <div className="w-16 h-16 bg-cyber-surface border border-neon-green mx-auto flex items-center justify-center">
            ...
            <div className="space-y-1 text-xs font-mono">
              <div className="flex justify-between">
                ...
                <span className="text-neon-yellow">
                  {formatAltitude(metadata.altitude)}m
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-cyber-muted">HFOV:</span>
                <span className="text-neon-green">
                  {formatAngle(metadata.hfov)}°
                </span>
              </div>
              <div className="flex justify-between">
